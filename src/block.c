...
struct block *get_block(struct huff_table *dc_table,
                            struct huff_table *ac_table,
                            struct bitstream *stream,
                            uint8_t *quantization_table,
                            int16_t previous_dc,
                            size_t size)
{
    ...
    int16_t *decompressed = decompress(dc_table,
                                        ac_table,
                                        stream,
                                        previous_dc,
                                        length);
    /* 2. Quantification inverse */
    int16_t *quantization = inverse_quantization(decompressed,
                                                    quantization_table,
                                                    length);
    free(decompressed);
    ...
    block->coefficients = idct(zigzag, size);
    free(zigzag);
    /* Retour du bloc */
    return block;
}
